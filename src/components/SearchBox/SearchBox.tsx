...
const SearchBox = () => {
    const [searchRequest, setSearchRequest, sendSearchRequest, state] =
        useSearch();
    const dispatch = useDispatch();
    const { createModal, removeFavoriteRequest, disableLike } =
    ...
    const likeState: likeStateType = useSelector(
        (state: RootState) => state.likeState
    );
    const [active, rightNowSetted] = [
        likeState.active,
        likeState.rightNowSetted,
    ];
    ...
    useEffect(() => {
        if (
            active &&
            (likeState.index >= favorites.length ||
                !isStateResultWiilBeSame(state)(favorites[likeState.index]))
        ) {
            disableLike();
        }
    }, [state]);
