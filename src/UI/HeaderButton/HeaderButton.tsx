...
const HeaderButton: React.FC<{statusType?: buttonStatusType, link: string, onClick?: () => void, type: buttonType}> = ({type, children, statusType= 'noStatus', link, onClick = () => {}}) => {
    const buttonStatusClass = buttonStatusClasses[statusType]
    const buttonTypeClass = buttonClasses[type]

    return (
        <Link onClick={onClick} className={`${styles.button} ${buttonStatusClass} ${buttonTypeClass}`} to={link}>
            {children}
        </Link>
  )
}
