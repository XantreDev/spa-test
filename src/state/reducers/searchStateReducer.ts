...
export function isSearchState(data: any): data is searchState{
    return isResultObject((data as searchState)?.result) && (data as searchState).searchRequest !== undefined
}
...
const searchStateReducer: (state: searchState | undefined, action: searchRequestAction) => searchState = (state = defaultState, action) => {
    switch (action.type) {
        ...
        case 'setOrderType':
            return {
                ...state,
                ordedBy: isOrderType(action.payload) ? action.payload : 'relevance'
            }
